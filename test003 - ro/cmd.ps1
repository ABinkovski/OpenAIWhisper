...
foreach ($model in $modelArray) {
    ...
    Measure-Command { 
		whisper $fileName --model $model --output_dir out/ro/$model --language ro --task translate --output_format all --condition_on_previous_text True --word_timestamps True  --fp16 False
	}
}
